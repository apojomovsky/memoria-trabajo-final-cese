...
Esta interfaz involucró la migración de la librería rosserial, mencionada en la sección \ref{sec:rosserial}, que hace posible entablar una comunicación serial asíncrona entre el microcontrolador y el una computadora corriendo ROS mediante su protocolo de mensajes estándar.

\begin{itemize}
  \item \textbf{Spin de ROS} esta tarea se encarga de llamar al método \file{spin} de la librería rosserial manera periódica. Esta cumple la función de \textit{heartbeat} para la comunicación con la computadora, donde ambas partes se indican mutuamente si existen mensajes o servicios que transmitir.
...
En esta sección se explica la estructura del meta-paquete Lubobot, el cual contiene las herramientas mínimas necesarias para comandar, modificar e inspeccionar el robot con ROS.
...
Este nodo se encuentra suscripto a los mensajes del tipo \file{lubo\_imu}, el cual se compone de las lecturas crudas de orientación y aceleración angular. Así, el programa se encarga de procesarlos a medida que llegan, agregandole información de tiempo y secuencia para luego republicarlos en el formato de mensaje estándar de ROS para unidades de medición inercial del tipo \file{sensor\_msgs/Imu}\protect\footnotemark.

El lector podría preguntarse por qué de no se eligió publicar el mensaje con el tipo estándar desde el microcontrolador directamente y ahorrarse la existencia de este nodo. La respuesta es que los mensajes estandares de ROS en general no resultan adecuados para dispositivos con poca memoria RAM y buses de transmisión como los que se encuentran típicamente en un microcontrolador. El uso de mensajes especiales diseñados específicamente para este fin, permitieron lograr un aumento en la frecuencia del mensaje y a su vez, la publicación de otros mensajes en simultáneo sin saturar el bus.

\footnotetext{\url{http://docs.ros.org/kinetic/api/sensor_msgs/html/msg/Imu.html}}
