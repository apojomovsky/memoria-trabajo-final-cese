...
% The ROS Gmapping package is a wrapper of the open source implementation of SLAM, called OpenSLAM (https://www.openslam.org/gmapping.html). The package contains a node called slam\_gmapping, which is the implementation of SLAM and helps to create a 2D occupancy grid map from the laser scan data and the mobile robot pose.
...
El paquete Gmapping para ROS es un \textit{wrapper} sobre la implementación de código abierto de la biblioteca de SLAM llamada OpenSLAM\protect\footnotemark. Este paquete ofrece un nodo llamado slam\_gmapping, que posibilita utilizar dentro del ecosistema ROS los algoritmos genéricos ofrecidos por la biblioteca.

GMapping permite generar el mapa del recinto en donde se encuentra el robot y así también, estimar la pose del robot dentro de dicho mapa.

Para utilizar GMapping en un robot, es necesario que el mismo disponga de:
\begin{itemize}
    \item un sensor del tipo \textit{range-finder} 2D fijado a la base del robot.
    \item una fuente de odometría
\end{itemize}

Para el robot propuesto en este trabajo, se utilizó como range\-finder un sensor Kinect, que como se menciona en la sección \ref{sec:kinect}, genera una imagen en 3D. Por este motivo se debió utilizar un nodo especial de ROS llamado depthimage_to_laserscan\protect\footnotemark para convertir las lecturas en 3D a otras en 2D antes de poder proveérselas a GMapping.
